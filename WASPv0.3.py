...
def recallSTW(chosen):
    ...
    info = {
        ...
        "Trade Effluent":None,
        "Per Capita Domestic Flow:":None,
        "Population Catchment":None,
        "BOD":None
        }
    # SAVE ALL FINAL FUNCTIONALITY INTO THIS DICTIONARY.
    
    # Presenting this to the output console.
    string = f"STW: {chosen}\n"
    for key in info.keys():
        string = string + f"{key}: {info[key]}\n"
    outputVar.delete("1.0", "end")
    outputVar.insert("1.0", string)
...
def updateSTW(chosen):
    ...
    if messagebox.askyesno(title="Save Changes", message="Are you sure wish to update these metrics?\nResults for PE, FFT and DWF may change."):   
        ...
        info = { # Retrieve information.
            "IDWF":idwfVal.get(),
            "Max Infiltration Rate":mirVal.get(),
            "Trade Effluent":tradeEffVal.get(),
            "Per Capita Domestic Flow:":perCapitaVal.get(),
            "Population Catchment":popCatchVal.get(),
            "BOD":bodVal.get()
            }
        # TODO functionality to update the database / STW with new values.
        try:
            return True
        except:
...
viewButton = tk.Button(metricPanel, text="Update STW", command=lambda: updateSTW(chosenSTW))
viewButton.grid(row=6, column=0)
